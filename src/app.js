...
function homeContent() {
  return `<div class="home-content yaru-welcome"><div class="welcome-intro">${appIcon('welcome')}<div><div class="hello-label">Hey there, I'm</div><h1>${escape(profile.name)}</h1><p class="welcome-role">${escape(profile.role)}</p></div></div><p class="welcome-description">${escape(profile.intro)}<br>Have a look around or get in touch.</p><button class="primary-button welcome-about" data-page="about">About me ${icon('arrow')}</button><h3 class="welcome-section-title">Explore this desktop</h3><div class="home-actions"><button class="home-action" data-page="projects">${appIcon('files')}<span><strong>Open source</strong><small>Tools and projects I've contributed to</small></span>${icon('arrow')}</button><button class="home-action" data-page="resume">${appIcon('resume')}<span><strong>Experience</strong><small>Research, engineering, education</small></span>${icon('arrow')}</button><button class="home-action" data-page="contact">${appIcon('contact')}<span><strong>Get in touch</strong><small>Email, LinkedIn, GitHub</small></span>${icon('arrow')}</button></div><div class="home-footer"><span>${icon('home')} ${escape(profile.location)}</span><a href="https://github.com/ubuntu/yaru" target="_blank" rel="noopener noreferrer">Yaru by the Ubuntu community ${icon('external')}</a></div></div>`;
}
...
function resumeContent() {
  return `<article class="page-content cv-content"><div class="eyebrow">EXPERIENCE & EDUCATION</div><div class="cv-title-row"><div><h2>Résumé</h2><p class="muted">${escape(profile.name)} · ${escape(profile.location)}</p></div><a class="primary-button" href="${escape(profile.resume)}" download="Utku_Egemen_Umut_Resume.pdf">${icon('file')} Download PDF</a></div><section class="cv-section"><h3>Education</h3><div class="cv-entry"><div class="cv-entry-heading"><strong>${escape(profile.education.institution)}</strong><span>${escape(profile.education.dates)}</span></div><p>${escape(profile.education.degree)} · ${escape(profile.education.program)}</p><p>${escape(profile.education.distinction)}</p><p class="muted">Relevant coursework: ${escape(profile.education.coursework)}</p></div></section><section class="cv-section"><h3>Experience</h3>${profile.experience.map(job => `<div class="cv-entry"><div class="cv-entry-heading"><strong>${escape(job.role)}</strong><span>${escape(job.dates)}</span></div><p class="cv-organization">${escape(job.organization)} · ${escape(job.location)}</p><ul>${job.highlights.map(point => `<li>${escape(point)}</li>`).join('')}</ul></div>`).join('')}</section><section class="cv-section"><h3>Open source</h3>${profile.openSource.map(project => `<div class="cv-entry"><div class="cv-entry-heading"><strong>${escape(project.name)}</strong></div><p>${escape(project.summary)}</p></div>`).join('')}</section><section class="cv-section"><h3>Teaching & leadership</h3><ul>${profile.leadership.map(item => `<li>${escape(item)}</li>`).join('')}</ul></section><section class="cv-section"><h3>Technical skills</h3>${profile.skillGroups.map(group => `<div class="cv-skill-group"><strong>${escape(group.name)}</strong><p>${group.items.map(escape).join(' · ')}</p></div>`).join('')}</section><a class="secondary-button" href="${escape(profile.resume)}" target="_blank" rel="noopener noreferrer">Open PDF in a new tab ${icon('external')}</a></article>`;
}
...
document.addEventListener('click', async e => {
  ...
  if (target.dataset.file) {
    ...
    else if (file.startsWith('project-')) { const project = profile.projects[Number(file.slice(8))]; if (project?.url && safeLink(project.url) !== '#') window.open(safeLink(project.url), '_blank', 'noopener,noreferrer'); }
